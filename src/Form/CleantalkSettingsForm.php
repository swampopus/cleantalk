...
function cleantalk_settings_form($form, &$form_state) {
  ...
  if ($config->get('cleantalk_show_renew_banner')) {

    $link = ($config->get('cleantalk_api_trial')) ? 'https://cleantalk.org/my/bill/recharge?utm_source=banner&utm_medium=wp-backend&utm_campaign=Backdrop%20backend%20trial&user_token=' : 'https://cleantalk.org/my/bill/recharge?utm_source=banner&utm_medium=wp-backend&utm_campaign=Backdrop%20backend%20renew&user_token=';

    backdrop_set_message(t("Cleantalk module trial period ends, please upgrade to <a href='" . $link . $config->get('cleantalk_api_user_token') . "' target='_blank'><b>premium version</b></a> ."), 'warning', false);
  }

  $form['#config'] = 'cleantalk.settings';
  $form['cleantalk_authkey'] = array(
    '#type' => 'textfield',
    '#title' => t('Access key'),
    '#size' => 20,
    '#maxlength' => 20,
    '#default_value' => $config->get('cleantalk_authkey'),
    '#description' => ($config->get('cleantalk_authkey'))?t('Account at cleantalk.org is <b>' . $config->get('cleantalk_api_account_name_ob') . '</b>') : t('Click <a target="_blank" href="!ct_link">here</a> to get access key.', array('!ct_link' => url('http://cleantalk.org/register?platform=backdrop'))),
  );
  ...
  $form['cleantalk_comments']['cleantalk_check_comments'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_comments'),
  ...
  $form['cleantalk_comments']['cleantalk_check_comments_automod'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_comments_automod'),
  ...
  $form['cleantalk_comments']['cleantalk_check_comments_min_approved'] = array(
    ...
    '#size' => 5,
    '#maxlength' => 5,
    '#default_value' => $config->get('cleantalk_check_comments_min_approved'),
    '#element_validate' => array('cleantalk_element_validate_integer_positive'),
  ...
  $form['cleantalk_search']['cleantalk_check_search_form'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_search_form'),
  ...
  $form['cleantalk_search']['cleantalk_add_search_noindex'] = array(
    ...
    '#default_value' => $config->get('cleantalk_add_search_noindex'),
  ...
  $form['cleantalk_exclusions']['cleantalk_url_exclusions_fieldset']['cleantalk_url_exclusions_container_inline']['cleantalk_url_exclusions'] = array(
    '#type' => 'textfield',
    '#default_value' => $config->get('cleantalk_url_exclusions'),
  ...
  $form['cleantalk_exclusions']['cleantalk_url_exclusions_fieldset']['cleantalk_url_exclusions_container_inline']['cleantalk_url_exclusions_regexp'] = array(
    '#type' => 'checkbox',
    '#title' => t('RegExp?'),
    '#default_value' => $config->get('cleantalk_url_exclusions_regexp'),
  );
  ...
  $form['cleantalk_exclusions']['cleantalk_fields_exclusions_fieldset']['cleantalk_fields_exclusions_container_inline']['cleantalk_fields_exclusions'] = array(
    '#type' => 'textfield',
    '#default_value' => $config->get('cleantalk_fields_exclusions'),
  );
  ...
  $form['cleantalk_exclusions']['cleantalk_roles_exclusions_fieldset']['cleantalk_roles_exclusions_container_inline']['cleantalk_roles_exclusions'] = array(
    ...
    '#multiple' => true,
    '#default_value' => $config->get('cleantalk_roles_exclusions'),
  );

  $form['cleantalk_check_register'] = array(
    '#type' => 'checkbox',
    '#title' => t('Check registrations'),
    '#default_value' => $config->get('cleantalk_check_register'),
  ...
  $form['cleantalk_check_webforms'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_webforms'),
  ...
  $form['cleantalk_check_contact_forms'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_contact_forms'),
  ...
  $form['cleantalk_check_forum_topics'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_forum_topics'),
  ...
  $form['cleantalk_check_added_content'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_added_content'),
  ...
  $form['cleantalk_check_ccf'] = array(
    ...
    '#default_value' => $config->get('cleantalk_check_ccf'),
  ...
  $form['cleantalk_set_cookies'] = array(
    '#type' => 'checkbox',
    '#title' => t('Set cookies'),
    '#default_value' => $config->get('cleantalk_set_cookies'),
    '#description' => t('Turn this option off to deny plugin generates any cookies on website front-end. This option is helpful if you use Varnish. But most of contact forms will not be protected if the option is turned off!') . '<br /><strong>' .
  ...
  $form['cleantalk_alternative_cookies_session'] = array(
    ...
    '#default_value' => $config->get('cleantalk_alternative_cookies_session'),
  ...
  $form['cleantalk_sfw'] = array(
    ...
    '#default_value' => $config->get('cleantalk_sfw'),
  ...
  $form['cleantalk_link'] = array(
    ...
    '#default_value' => $config->get('cleantalk_link'),
...
function cleantalk_settings_form_validate($form, &$form_state) {
  ...
  if ($form_state['values']['cleantalk_authkey']) {
    ...
    if (isset($is_valid['valid']) && intval($is_valid['valid']) == 1) {
      ...
      if (empty($account_status['error'])) {
        config_set('cleantalk.settings', 'cleantalk_api_show_notice', isset($account_status['show_notice']) ? $account_status['show_notice'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_renew', isset($account_status['renew']) ? $account_status['renew'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_trial', isset($account_status['trial']) ? $account_status['trial'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_user_token', isset($account_status['user_token']) ? $account_status['user_token'] : '');
        config_set('cleantalk.settings', 'cleantalk_api_spam_count', isset($account_status['spam_count']) ? $account_status['spam_count'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_moderate_ip', isset($account_status['moderate_ip']) ? $account_status['moderate_ip'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_moderate', isset($account_status['moderate']) ? $account_status['moderate'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_show_review', isset($account_status['show_review']) ? $account_status['show_review'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_service_id', isset($account_status['service_id']) ? $account_status['service_id'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_license_trial', isset($account_status['license_trial']) ? $account_status['license_trial'] : 0);
        config_set('cleantalk.settings', 'cleantalk_api_account_name_ob', isset($account_status['account_name_ob']) ? $account_status['account_name_ob'] : '');
        config_set('cleantalk.settings', 'cleantalk_api_ip_license', isset($account_status['ip_license']) ? $account_status['ip_license'] : 0);
        config_set('cleantalk.settings', 'cleantalk_show_renew_banner', ($config->get('cleantalk_api_show_notice') && $config->get('cleantalk_api_trial')) ? 1 : 0);
      }
      if (intval($form_state['values']['cleantalk_sfw']) === 1) {
        $sfw = new CleantalkSFW();
        $sfw->sfw_update($cleantalk_auth_key);
        $sfw->send_logs($cleantalk_auth_key);
        config_set('cleantalk.settings', 'cleantalk_sfw_last_logs_sent', time());
        config_set('cleantalk.settings', 'cleantalk_sfw_last_updated', time());
      }
      // Turns off alternative cookies setting if cookies are disabled
      if (intval($form_state['values']['cleantalk_set_cookies']) == 0 ) {
...
function cleantalk_regexp_validation($element, &$form_state, $form ) {
  ...
  if ( $form_state['values']['cleantalk_url_exclusions_regexp'] ) {
    ...
    if ( ! empty($errors) ) {
      ...
      config_set('cleantalk.settings', 'cleantalk_url_exclusions', '');
