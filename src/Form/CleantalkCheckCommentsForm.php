...
function cleantalk_check_comments_form($form, &$form_state) {
  if (module_exists('comment')) {
    if (config_get('cleantalk.settings', 'cleantalk_authkey') != '') {
      $rows = array();
      $header = array(
        ...
        foreach ($form_state['spam_comments'] as $spam_comment) {
          $rows[] = array(
            ...
            '#attributes' => array(
              ...
              'class' => array('cleantalk-spam-comments-row'),
            ),
          );
        }
      }
      ...
      backdrop_set_message('Access key is not valid.', 'error');
    }
  }
  else {
    backdrop_set_message('Comments module is disabled.', 'error');
  }
}
...
function cleantalk_find_spam_comments($comments) {
  ...
  if ($comments && count($comments) > 0) {
    ...
    $data = implode(',', $data);
    $result = CleantalkHelper::api_method__spam_check_cms(trim(config_get('cleantalk.settings', 'cleantalk_authkey')), $data);

    if (isset($result['error_message'])) {
      backdrop_set_message($result['error_message'], 'error');
    }
    else {
